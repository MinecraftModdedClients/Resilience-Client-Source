...
public class GuiAdd extends GuiScreen{
	...
	public GuiAdd(GuiScreen screen){
		parent = screen;
	}
	...
	public void initGui()
	{
		...
		this.buttonList.add(new ResilienceButton(0, Resilience.getInstance().getInvoker().getWidth()/2 - 102, Resilience.getInstance().getInvoker().getHeight() /2 +25, 204, 20, "Add"));
	...
	protected void actionPerformed(GuiButton par1)
	{
		...
		if(par1.id == 0)
		{
			Account.accountList.add(new Account(usernameBox.getText(), passwordBox.getText()));
